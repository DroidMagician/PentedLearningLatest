package com.pented.learningapp.homeScreen.scanQR.activity
...
import com.pented.learningapp.databinding.ActivityWatchScannedVideoBinding
...
class WatchQRVideoActivity : BaseActivity<ActivityWatchScannedVideoBinding>() {
    private val b get() = BaseActivity.binding as ActivityWatchScannedVideoBinding
    ...
    override fun layoutID() = R.layout.activity_watch_scanned_video
       override fun viewModel(): BaseViewModel = ViewModelProvider(this).get(WatchScannedVideoVM::class.java)
    ...
    private fun listner() {
        b.btnExit.setOnClickListener {
                ...
                watchVideoVM.earnPointsRequestModel.SubjectId = Constants.subjectIdForScanQR
...
//                        }
    ...
    private fun init() {
        ...
        if(intent.hasExtra("topicVideo"))
        {
            ...
            val gson = Gson()
            val yourObject = gson.fromJson<ScanQrResponseModel>(intent.getStringExtra("topicVideo"), ScanQrResponseModel::class.java)
            b.txtPoints.text = "${yourObject.data?.Points} Points"
            topicId = yourObject.data?.TopicId ?: 0
            Log.e("yourObject","Is Here ${Gson().toJson(yourObject)}")
            b.txtTopicName.text = yourObject.data.TopicVideoTitle
            b.progressView1.progress = yourObject?.data?.CompletedPercentage?.toFloat()!!
            Executors.newSingleThreadExecutor().submit(Runnable {
                // You can perform your task here.
                // Log.e("UUID", "Is ${getDeviceId(this@GetStartedActivity)}")
                var s3Client = S3Util.getS3Client()
...
//                        }
                ...
                val request = GeneratePresignedUrlRequest(
                    "pentedapp",
                    "${yourObject.data.S3Bucket?.BucketFolderPath}${yourObject.data.S3Bucket?.FileName}"
                )
                val objectURL: URL = s3Client.generatePresignedUrl(request)
                Log.e("Final URL is", "Here $objectURL")
                //var clip = Uri.parse(objectURL.toString().replace(" ", "%20"))
            ...
            if(yourObject.data.S3Bucket != null && (yourObject.data.S3Bucket?.FileName != null))
            {
                b.youTubePlayerView.visibility = View.GONE
                b.playerView.visibility = View.VISIBLE
                Handler(Looper.getMainLooper()).postDelayed({
                    initializePlayer(yourObject.data.S3Bucket?.FileName ?: "")
                }, 1000)
            }
            else if(!yourObject.data.Youtubelink.isNullOrBlank())
            {
                var youTubeId:String ? = null
                var listStrings = yourObject.data.Youtubelink?.split("=")
...
//                getLifecycle().addObserver(youTubePlayerView)
                ...
                b.youTubePlayerView.addYouTubePlayerListener(object :
                    ...
                    override fun onReady(@NonNull youTubePlayer: YouTubePlayer) {
                        ...
                        // ✅ Apply equivalent of your XML attributes here
...
//                })
                ...
                Toast.makeText(this@WatchQRVideoActivity,"No Video Found", Toast.LENGTH_SHORT).show()
            }
...
//                    }, 1000)
    ...
    fun showCongratulationsDialog(quotient: Int) {
        val dialog = Dialog(this@WatchQRVideoActivity)
...
//                watchVideoVM.addPoints()
    ...
    private fun initPopupQuality() {
        ...
        val trackSelectionDialogBuilder = CustomTrackSelectionDialogBuilder(this, getString(R.string.qualitySelector), trackSelector, videoRenderer)
